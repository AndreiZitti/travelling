...
export default function FlatMap({
    ...
    fetch(TOPOJSON_URL)
      ...
      .then((topology: Topology<{ countries: GeometryCollection<CountryProperties> }>) => {
        ...
        const getCountryName = (id: string): string => {
          // Map of numeric IDs to country names (from Natural Earth)
        ...
        svg
          ...
          .attr("fill", (d) => {
            const name = getCountryName(String(d.id));
            const code = getCountryCode(name);
            return code && isVisited(code) ? "#6366f1" : "#f5f5f5";
          })
          .attr("stroke", "#e5e5e5")
          ...
          .on("mouseover", function (event, d) {
            ...
            d3.select(this).attr("fill", visited ? "#4f46e5" : "#e5e5e5");
            setTooltip({
              x: event.pageX,
              y: event.pageY - 10,
              name: name || "Unknown",
          ...
          .on("mouseout", function (_, d) {
            ...
            const code = getCountryCode(name);
            const visited = code && isVisited(code);
            d3.select(this).attr("fill", visited ? "#6366f1" : "#f5f5f5");
          ...
          .on("click", function (_, d) {
            ...
            if (code) {
              ...
              d3.select(this).attr("fill", nowVisited ? "#6366f1" : "#f5f5f5");
            }
          });
      })
  ...
  return (
    <div ref={containerRef} className="w-full h-full map-container relative">
